...
The scan classification logic, box detection patterns, PSLIP guard, order TTL, and all timing constants are identical to TM v5.4.
...
_WatchPackApp() [100ms timer]
      |
      | WinGetText(active Firefox) -> StrUpper -> regex match for PB2/PM4/PM5/OWNBOX/SIOC
      |
      | box found -> _WriteOrder() -> %TEMP%\fc_auto_order.json
      |
_PollShipApp() [400ms timer]
      |
...
Timing values mirror TM v5.4 exactly. `A_Temp` resolves to the user's `%TEMP%` directory, which is always writable without admin rights.
...
Mirrors TM v5.4 `onScan()` exactly, with one addition: a **barcode gate** that pre-filters manual keyboard input:
...
| Pattern | Action |
|---|---|
| Not SP00, not PSLIP, tote not yet seen | `g_toteScanned = true`, tip shown |
...
### _WatchPslip (600ms timer)

Reads the active Firefox window's MSAA text every 600ms. Sets/clears `g_pslipVisible` and manages the persistent PSLIP ToolTip in slot 2:
...
`StrUpper` normalizes before matching so `\b` word-boundary matching works regardless of original case. `\b` prevents partial matches (e.g. "OWNBOXES" falsely matching "OWNBOX").
...
Step 4: _WaitForWindowText(["SUCCESS", "success", "FAILURE", "Invalid"], 10s)
```

`WinActivate` brings the ShipApp Firefox window to the foreground. This is necessary because `SendText` sends keystrokes to the **active window** -- unlike the TM version which dispatches events directly to a DOM node regardless of tab visibility.

**PSLIP note:** ShipApp does NOT require a PSLIP barcode (confirmed live test 2026-06-10). The flow is always SP00 -> BOX -> label prints.
...
```json
{"sp00":"spRKB6Xg4MW","box":"PB2","barcode":"FSA","ts":12345678}
```
...
```autohotkey
global POLL_MS         := 400      ; how often ShipApp polls for orders (ms)
global PSLIP_POLL_MS   := 600      ; PSLIP prompt check interval (ms)
...
If ShipApp starts timing out after SP00 or the box scan on a slow day, increase `BOX_SEND_DELAY` in 200ms steps. If PackApp box detection stops working reliably, increase `BOX_DETECT_MAX`.
...
`_WatchPslip` only runs when a Firefox window is active. If PSLIP text appears in PackApp but the tip never shows:
