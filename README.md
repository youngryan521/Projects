...
|---|---|
| Not SP00, not PSLIP pattern, tote not yet scanned | Mark tote as scanned, ignore |
...
5. `waitFor(SUCCESS or FAILURE, 10s)` -- waits for result and reports

PSLIP is confirmed NOT required in ShipApp (tested 2026-06-10). The flow is always SP00 -> BOX -> label prints regardless of order type.
...
### v5.4 -- Performance Optimization (current)
...
- Data stored: `{ sp00, box, barcode, ts }` -- order routing identifiers only
